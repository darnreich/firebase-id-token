...
describe FirebaseIDToken::Validator do
  ...
  describe '#check' do
    ...
    it 'should successfully validate against a passed-in X509 cert' do
      literal_validator = FirebaseIDToken::Validator.new(x509_cert: @cert)
    ...
    context 'with old_skool certs' do
      let(:validator) { FirebaseIDToken::Validator.new }

      context 'when unable to fetch old_skool Firebase certs' do
        before do
      ...
      context 'when able to fetch old_skool certs' do
        ...
        context 'when certificates are expired' do
          let(:validator) { FirebaseIDToken::Validator.new(expiry: 60) }
